...
fun main() {
    println(day8Part1(day8TestInput))
    println(day8Part1(day8PuzzleInput))
    println(day8Part2(day8TestInput))
    println(day8Part2(day8PuzzleInput))
}

fun day8Part1(input: String): Int {
    // Count words of length 2,3,4,7
    val wordList = input.split("\n").map { it.split(" | ")[1].split(" ") }.flatten()
    return wordList.count { it.length == 2 || it.length == 3 || it.length == 4 || it.length == 7 }
}
...
const val day8PuzzleInput = """fdb bgfa cedfg abedcf defgba gbaed dagfbce fbged dgabce fb | bf dfbega fb baged
